...
class Builder:
    ...
    def show_image(self, xpath):
        ...
        #values = {'action':'show_image', 'imgfile':self.node_imgfile}
        query = '%s?%s' % (sys.argv[0], urllib.urlencode(values))
    ...
    def show_text(self, xpath):
        ...
        #values = {'action':'show_text', 'txtfile':self.node_txtfile, 'title':self.driver.title.encode('utf-8')}
        query = '%s?%s' % (sys.argv[0], urllib.urlencode(values))
        query = 'RunPlugin(%s)' % query
        return (label, query)
    ...
    def current_page(self):
        ...
        page_imgfile = self.page_imgfile
        item = xbmcgui.ListItem(label, iconImage=page_imgfile, thumbnailImage=page_imgfile)
        # コンテクストメニューを設定
        context_menu = []
        item.addContextMenuItems(context_menu, replaceItems=True)
        # アイテムを追加
        values = {'action':'show_image', 'imgfile':page_imgfile}
        query = '%s?%s' % (sys.argv[0], urllib.urlencode(values))
    ...
    def parent_node(self):
        ...
        node_imgfile = os.path.join(self.cachedir, '%s_%s.png' % (self.driver.session_id,hashlib.md5(xpath).hexdigest()))
        if not os.path.isfile(node_imgfile):
            self.chrome.save_image(node_imgfile, element=elem)
        item = xbmcgui.ListItem(label, iconImage=node_imgfile, thumbnailImage=node_imgfile)
        # コンテクストメニュー
        context_menu = []
        #### ノードリストを表示する
        ...
        item.addContextMenuItems(context_menu, replaceItems=True)
        # アイテムを追加
        values = {'action':'show_image', 'imgfile':node_imgfile}
        query = '%s?%s' % (sys.argv[0], urllib.urlencode(values))
        xbmcplugin.addDirectoryItem(int(sys.argv[1]), query, item, False)
    ...
    def current_node(self):
        ...
        node_imgfile = self.node_imgfile
        item = xbmcgui.ListItem(label, iconImage=node_imgfile, thumbnailImage=node_imgfile)
        # コンテクストメニュー
        context_menu = []
        #### ノードリストを表示する
    ...
    def nodelist(self, xpath):
        ...
        for i in range(0,len(elems)):
            ...
            if height>0 and width>0 and height<ua_height and width<ua_width and (height<ua_height/2 or width<ua_width/2):
                ...
                xpath2 = xpath1
                # キャプチャを取得
                imgfile = os.path.join(self.cachedir, '%s_%s.png' % (self.driver.session_id,hashlib.md5(xpath2).hexdigest()))
                if not os.path.isfile(imgfile):
                    self.chrome.save_image(imgfile, element=elems[i])
                # テキストを取得
                txtfile = os.path.join(self.cachedir, '%s_%s.txt' % (self.driver.session_id,hashlib.md5(xpath2).hexdigest()))
                if not os.path.isfile(txtfile):
                    self.chrome.save_text(txtfile, element=elems[i])
                # ラベル
                ...
                if text:
                    ...
                    values = {'action':'show_image', 'imgfile':imgfile}
                    query = '%s?%s' % (sys.argv[0], urllib.urlencode(values))
                    list.append((label, query, imgfile, txtfile, context_menu))
            else:
...
class Google(Builder):
    ...
    def extract(self, keyword):
        ...
        for div in heading:
            ...
            list.append((label, query))
        # 画面全体をキャプチャ
        self.page_imgfile = os.path.join(self.cachedir, '%s.png' % (self.driver.session_id))
        if not os.path.isfile(self.page_imgfile):
            self.chrome.save_image(self.page_imgfile)
        # 親ページを表示する
        self.current_page()
        # リンクのリスト
...
class Browser(Builder):
    ...
    def extract(self, xpath=None, target=None, imgfile=None, txtfile=None, wavfile=None):
        ...
        # 指定部分をキャプチャ
        self.node_imgfile = os.path.join(self.cachedir, '%s_%s.png' % (self.driver.session_id, hashlib.md5(self.xpath).hexdigest()))
        if not os.path.isfile(self.node_imgfile):
            self.chrome.save_image(self.node_imgfile, element=self.elem)
        # 指定部分のテキスト
        self.node_txtfile = os.path.join(self.cachedir, '%s_%s.txt' % (self.driver.session_id, hashlib.md5(self.xpath).hexdigest()))
        if not os.path.isfile(self.node_txtfile):
            self.chrome.save_text(self.node_txtfile, element=self.elem)
        # 指定部分のテキストを音声合成（ファイルパスのみ設定）
        self.node_wavfile = os.path.join(self.cachedir, '%s_%s.wav' % (self.driver.session_id, hashlib.md5(self.xpath).hexdigest()))
        # 画面表示
        if self.target == self.TARGET_NODELIST:
            ...
            self.__extract_files(imgfile, txtfile, wavfile)
        # ノード情報を返す
        return {
            'url': self.url,
            'xpath': self.xpath,
            ...
            'page_imgfile': self.page_imgfile,
            'node_imgfile': self.node_imgfile,
            'node_txtfile': self.node_txtfile
        }
    ...
    def __extract_nodelist(self):
        ...
        self.current_node()
        # 子ノードのリスト
    	for (label, query, imgfile, txtfile, context_menu) in self.nodelist(self.xpath):
            item = xbmcgui.ListItem(label, iconImage=imgfile, thumbnailImage=imgfile)
            item.addContextMenuItems(context_menu, replaceItems=True)
            xbmcplugin.addDirectoryItem(int(sys.argv[1]), query, item, True)
        xbmcplugin.endOfDirectory(int(sys.argv[1]))
    ...
    def __extract_image(self):
        show_image(self.node_imgfile)
        xbmcplugin.endOfDirectory(int(sys.argv[1]), succeeded=False)

    def __extract_text(self):
        show_text(self.node_txtfile, self.driver.title.encode('utf-8'))
        xbmcplugin.endOfDirectory(int(sys.argv[1]), succeeded=False)

    def __extract_wav(self, play=True):
        # 指定部分のテキストから音声合成
        if not os.path.isfile(self.node_wavfile):
            # ファイル読み込み
            if os.path.isfile(self.node_txtfile):
                f = open(self.node_txtfile,'r')
                data = f.read()
                f.close()
            else:
                data = ''
            # 音声合成を実行
            values = {'text':data.replace('\n',' '), 'silent':'true', 'wavfile':self.node_wavfile}
            postdata = urllib.urlencode(values)
            xbmc.executebuiltin('RunPlugin(plugin://script.handler.tts/?%s)' % (postdata))
            # WAVFILEの生成を待機
            while not os.path.exists(self.node_wavfile):
                xbmc.sleep(1)
        if play:
            xbmc.executebuiltin('PlayMedia(%s)' % self.node_wavfile)
            #show_image(self.node_imgfile)
            show_text(self.node_txtfile, self.driver.title.encode('utf-8'))
        xbmcplugin.endOfDirectory(int(sys.argv[1]), succeeded=False)
    ...
    def __extract_files(self, imgfile=None, txtfile=None, wavfile=None):
        if imgfile and os.path.isfile(self.node_imgfile):
            shutil.copyfile(self.node_imgfile, imgfile)
        if txtfile and os.path.isfile(self.node_txtfile):
            shutil.copyfile(self.node_txtfile, txtfile)
        if wavfile:
            self.__extract_wav(play=False)
            shutil.copyfile(self.node_wavfile, wavfile)
